...
class LegendWidget extends StatefulWidget {
  ...
  const LegendWidget({required this.dataSource, required this.settings});
            ...
            return Column(
              mainAxisAlignment:
                  widget.settings.position == TreemapLegendPosition.bottom
                      ? MainAxisAlignment.end
                      : MainAxisAlignment.start,
              children: <Widget>[widget.settings.title!, actualChild],
            );
          } else {
            return Column(
              mainAxisAlignment: MainAxisAlignment.center,
  ...
  List<Widget> _getLegendItems() {
    ...
    if (widget.dataSource != null) {
      ...
      if (widget.dataSource is List) {
        final List<TreemapColorMapper> source =
            // ignore: avoid_as
            widget.dataSource as List<TreemapColorMapper>;
        if (source.isNotEmpty) {
          final int length = source.length;
          for (int i = 0; i < length; i++) {
            final TreemapColorMapper colorMapper = source[i];
...
class _SolidBarLegendState extends State<_SolidBarLegend> {
  ...
  List<Widget> _getBarSegments() {
    if (widget.dataSource != null) {
      if (widget.dataSource is List) {
        ...
        final Map<String, TreemapTile> source =
            // ignore: avoid_as
            widget.dataSource as Map<String, TreemapTile>;
        if (source.isNotEmpty) {
          _labelsPlacement = widget.settings._labelsPlacement ??
  ...
  List<Widget> _getSegmentsForColorMapper() {
    ...
    for (int i = 0; i < length; i++) {
      ...
      if (_labelsPlacement == TreemapLegendLabelsPlacement.betweenItems) {
        if (i == length - 1) {
          currentText =
              _getTrimmedText(_getText(source[i]), currentText, i, length);
        } else {
          if (i == 0) {
          ...
          currentText =
              _getTrimmedText(currentText, _getText(source[i + 1]), i, length);
        }
      } else {
        currentText = _getText(colorMapper);
  ...
  List<Widget> _getSegmentsForShapeSource() {
    ...
    final Iterator<TreemapTile> currentIterator = source.values.iterator;
    final Iterator<TreemapTile> nextIterator = source.values.iterator;
    String? text;
  ...
  List<String> _getStartSegmentLabel(TreemapColorMapper colorMapper) {
    if (colorMapper.from != null &&
      ...
      return <String>[_getText(colorMapper)];
    }
  }
...
class _GradientBarLegend extends StatelessWidget {
  ...
  void _collectLabelsAndColors() {
    ...
    if (dataSource != null) {
      if (dataSource is List) {
        final List<TreemapColorMapper> source =
            // ignore: avoid_as
            dataSource as List<TreemapColorMapper>;
        if (source.isNotEmpty) {
          _collectColorMapperLabelsAndColors(source.length);
        }
      } else {
        final Map<String, TreemapTile> source =
            // ignore: avoid_as
            dataSource as Map<String, TreemapTile>;
        if (source.isNotEmpty) {
          final int length = source.length;
          String? text;
          double slab;
          ...
          // third item is overlapping or not. Since the iterator in
          // second model.So we uses two iterator. If we use move next first
          // iterator givesurrent model and second iterator gives next model.
          final Iterator<TreemapTile> currentIterator = source.values.iterator;
          final Iterator<TreemapTile> nextIterator = source.values.iterator;
          int index = 0;
          nextIterator.moveNext();
          while (currentIterator.moveNext()) {
            ...
            if (_labelsPlacement == TreemapLegendLabelsPlacement.betweenItems) {
              if (nextIterator.moveNext()) {
                text = _getTrimmedText(treemapModel.group, positionIndex,
                    length, slab, nextIterator.current.group);
              } else {
                text = _getTrimmedText(
      ...
      for (int i = 0; i < length; i++) {
        ...
        if (source[0].from != null) {
          ...
          if (_labelsPlacement == TreemapLegendLabelsPlacement.onItem) {
            ...
            text = _getTrimmedText(
                text, i, length, slab, _getActualText(source[i + 1]));
          }
  ...
  List<String> _getStartSegmentLabel(TreemapColorMapper colorMapper) {
    if (colorMapper.from != null &&
      ...
      return <String>[_getActualText(colorMapper)];
    }
  }
