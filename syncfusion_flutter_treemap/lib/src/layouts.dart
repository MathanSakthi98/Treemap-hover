...
class _TreemapState extends State<Treemap> {
  final Color _baseColor = const Color.fromRGBO(42, 80, 160, 1);
...
class _SquarifiedTreemapState extends State<_SquarifiedTreemap> {
  ...
  List<Widget> _getTileWidgets(List<TreemapTile> source, Size size, Axis axis,
      Offset offset, int start, int end) {
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        return widget.type == LayoutType.slice
            ...
            : Row(textDirection: TextDirection.ltr, children: _children);
      },
    );
  }
}
...
class _TileDecorState extends State<_TileDecor> with TickerProviderStateMixin {
  ...
  RoundedRectangleBorder? _getBorder() {
    ...
    if (_isHover && widget.details == widget.treemapController.hoveredTile) {
      border = _ancestor.tileHoverBorder;
    } else if (_isSelected &&
        widget.details == widget.treemapController.selectedTile) {
  ...
  Widget build(BuildContext context) {
    ...
    final RoundedRectangleBorder? _border = _getBorder();
